...
{{- else -}}
// {{ func_name_context $q }} runs a custom query{{ if $q.Exec }} returning command tag{{ else if not $q.Flat }}, returning results as [{{ $q.Type.GoName }}]{{ end }}.
{{- end }}
{{ func_context $q }} {
	// query
	{{ querystr $q }}
...
{{- else -}}
// {{ func_name $q }} runs a custom query{{ if $q.Exec }} returning command tag{{ else if not $q.Flat }}, returning results as [{{ $q.Type.GoName }}]{{ end }}.
{{- end }}
{{ func $q }} {
