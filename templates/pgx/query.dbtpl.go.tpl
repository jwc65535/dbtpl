...
{{- else -}}
// {{ func_name_context $q }} runs a custom query{{ if $q.Exec }} as a [pgconn.CommandTag]{{ else if not $q.Flat }}, returning results as [{{ $q.Type.GoName }}]{{ end }}.
{{- end }}
{{ func_context $q }} {
	// query
	{{ querystr $q }}
...
{{- else -}}
        ...
        return Collect(rows, func(r pgx.Rows) ({{ $q.Type.GoName }}, error) {
                var {{ short $q.Type}} {{ type $q.Type.GoName }}
                err := r.Scan({{ names (print "&" (short $q.Type) ".") $q.Type.Fields }})
                return {{ short $q.Type }}, err
        })
{{- end }}
}
...
{{- else -}}
// {{ func_name $q }} runs a custom query{{ if $q.Exec }} as a [pgconn.CommandTag]{{ else if not $q.Flat }}, returning results as [{{ $q.Type.GoName }}]{{ end }}.
{{- end }}
{{ func $q }} {
