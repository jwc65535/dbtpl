...
func Init(ctx context.Context, f func(xo.TemplateType)) error {
	...
	f(xo.TemplateType{
		...
		Flags: []xo.Flag{
			...
			{
				ContextKey: PgxPoolKey,
				Type:       "bool",
				Desc:       "use pgxpool instead of direct connection",
				Default:    true,
			},
			{
...
func PgxGoType(d xo.Type, schema, intType, _ string) (string, string, error) {
	...
	case "smallint", "int2":
		if typNullable {
			return "pgtype.Int2", "pgtype.Int2{}", nil
		}
		return "int16", "0", nil
	...
	case "boolean", "bool":
		if typNullable {
			return "pgtype.Bool", "pgtype.Bool{}", nil
		}
...
func (f *Funcs) FuncMap() template.FuncMap {
	return template.FuncMap{
		...
		"imports":     f.importsfn,
		"inject":      f.injectfn,
		"use_pgxpool": func() bool { return f.usePgxPool },
		// context
		"context":         f.contextfn,
		...
		"querystr": f.querystr,
		"sqlstr":   f.sqlstr,
		"has_autogenerated_pk": func(v any) bool {
			return f.sequencePrimaryKey(v) != nil
		},
		"seq_field": func(v any) Field {
			if seq := f.sequencePrimaryKey(v); seq != nil {
				return *seq
			}
			return Field{}
		},
...
// sequencePrimaryKey returns the first sequence field that is also the sole primary key.
func (f *Funcs) sequencePrimaryKey(v any) *Field {
	switch t := v.(type) {
	case Table:
...
var (
	...
	PgxPoolKey    xo.ContextKey = "pgx-pool"
)
...
// UsePgxPool returns pgx-pool from the context.
func UsePgxPool(ctx context.Context) bool {
	b, _ := ctx.Value(PgxPoolKey).(bool)
	return b
}
