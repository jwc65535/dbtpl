...
func goType(f xo.Field) string {
	hasDefault := f.Default != "" && !f.IsSequence
	isNullable := f.Type.Nullable

	if f.IsSequence {
		switch strings.ToLower(f.Type.Type) {
	...
	if hasDefault {
		return pointerPgType(f.Type)
	}

	if !isNullable {
		return nativeGoType(f.Type)
	}

	return pointerPgType(f.Type)
}

func nativeGoType(t xo.Type) string {
	if t.IsArray {
		return "[]" + nativeScalarType(t.Type)
	}
	return nativeScalarType(t.Type)
}

func nativeScalarType(sqlType string) string {
	...
	case "double precision", "float8":
		return "float64"
	case "text", "varchar", "char", "character varying", "character", "name":
		return "string"
	case "bytea":
		return "[]byte"
	case "date":
...
func pointerPgType(t xo.Type) string {
	baseType := pgTypeMapping(t.Type)
	if t.IsArray {
		scalarType := nativeScalarType(t.Type)
		return "*[]" + scalarType
	}
	return "*" + baseType
}

func pgTypeMapping(sqlType string) string {
	switch strings.ToLower(sqlType) {
	case "boolean", "bool":
		return "pgtype.Bool"
	case "smallint", "int2":
