...
{{ recv_context $t "Insert" }} {
	...
	{{- if not .IsSequence }}
	{{- if has_default . }}
	if {{ short $t }}.{{ .GoName }} != nil {
		cols = append(cols, "{{ colname . }}")
		vals = append(vals, {{ short $t }}.{{ .GoName }})
	...
	{{- end }}
	{{- end }}
	sqlstr := fmt.Sprintf("INSERT INTO {{ schema $t.SQLName }} (%s) VALUES (%s)",
		strings.Join(cols, ", "),
		strings.Join(placeholders, ", "))
	{{- if has_sequence $t }}
	sqlstr += " RETURNING {{ (seq_field $t).SQLName }}"
	logf(sqlstr, vals...)
	if err := {{ db "QueryRow" "vals..." }}.Scan(&{{ short $t }}.{{ (seq_field $t).GoName }}); err != nil {
		return logerror(err)
	}
	{{- else }}
	logf(sqlstr, vals...)
	if _, err := {{ db "Exec" "vals..." }}; err != nil {
		return logerror(err)
	}
	{{- end }}
...
{{ recv_context $t "Update" }} {
	...
	{{- if has_default . }}
	if {{ short $t }}.{{ .GoName }} != nil {
		sets = append(sets, fmt.Sprintf("{{ colname . }} = $%d", paramNum))
		vals = append(vals, {{ short $t }}.{{ .GoName }})
		paramNum++
	}
	{{- else }}
	...
	var where []string
	{{- range $t.PrimaryKeys }}
	where = append(where, fmt.Sprintf("{{ colname . }} = $%d", paramNum))
	vals = append(vals, {{ short $t }}.{{ .GoName }})
	paramNum++
	{{- end }}
	sqlstr := fmt.Sprintf("UPDATE {{ schema $t.SQLName }} SET %s WHERE %s",
		strings.Join(sets, ", "),
...
{{ recv_context $t "Upsert" }} {
	...
	if {{ short $t }}.{{ .GoName }} != nil {
		...
		placeholders = append(placeholders, fmt.Sprintf("$%d", paramNum))
		{{- if not .IsPrimary }}
		updateSets = append(updateSets, "{{ colname . }} = EXCLUDED.{{ colname . }}")
		{{- end }}
		paramNum++
	}
	{{- else }}
	...
	{{- end }}
	{{- end }}
	sqlstr := fmt.Sprintf("INSERT INTO {{ schema $t.SQLName }} (%s) VALUES (%s) ON CONFLICT ({{ range $i, $pk := $t.PrimaryKeys }}{{ if $i }}, {{ end }}{{ colname $pk }}{{ end }}) DO UPDATE SET %s",
		strings.Join(cols, ", "),
		strings.Join(placeholders, ", "),
