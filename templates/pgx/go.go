	...
	f(xo.TemplateType{
		...
		Flags: []xo.Flag{
			...
			},
			{
				...
				Type:       "bool",
				Desc:       "use pgxpool instead of pgx.Conn",
				Default:    true,
			},
			{
				ContextKey: UUIDKey,
...
func (f *Funcs) usepgxpoolfn() bool {
	return f.pgxPool
}
...
// db generates a db.<name>(ctx, sqlstr, ...)
...
// db_prefix generates a db.<name>(ctx, sqlstr, <prefix>.param, ...).
//
